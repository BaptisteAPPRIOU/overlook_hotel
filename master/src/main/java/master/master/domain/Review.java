...
/** JPA Entity for Review. Represents room reviews in the database. */
@Entity
@Table(
    name = "reviews",
    uniqueConstraints = {@UniqueConstraint(columnNames = {"room_id", "author_id"})})
@Data
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class Review {
  ...
  // Composite unique constraint to prevent duplicate reviews from same user for
  // same room
  ...
  public boolean canBeEditedBy(Long userId) {
    return authorId != null
        ...
        && createdAt != null
        && createdAt.isAfter(LocalDateTime.now().minusDays(7)); // Can edit within 7 days
  }
