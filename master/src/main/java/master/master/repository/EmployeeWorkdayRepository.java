...
/**
 * Repository interface for managing EmployeeWorkday entities. Extends JpaRepository to provide
 * basic CRUD operations for EmployeeWorkday entities with composite key WorkdayId.
 *
 * <p>This repository handles the persistence layer operations for employee workday assignments,
 * allowing queries and modifications based on employee user IDs.
 */
public interface EmployeeWorkdayRepository extends JpaRepository<EmployeeWorkday, WorkdayId> {
  ...
  /**
   * Delete all workdays for a specific employee by their user ID. This method requires @Modifying
  ...
  /**
   * Find all workdays with their associated employee and user data. Uses JOIN FETCH to avoid N+1
   * query problems.
   *
  ...
  /**
   * Count distinct employees who have workdays configured. Based on the WorkdayId structure, we
   * access the employeeId through the composite key.
   *
  ...
  /**
   * Find workdays for multiple employees by their user IDs. Useful for bulk operations.
   *
  ...
  /**
   * Find employees who have workday configurations. Returns the distinct employee user IDs.
   *
  ...
  /**
   * Find workday numbers (1-7) for a specific employee. This method extracts the day of week from
   * workdates and returns them as integers.
   *
  ...
  @Query(
      value =
          "SELECT DISTINCT EXTRACT(dow FROM ew.work_date) FROM employee_workday ew "
  ...
  List<EmployeeWorkday> findByIdWorkDateBetween(
   ...
   */
  @Query(
      "SELECT ew FROM EmployeeWorkday ew WHERE ew.employee.userId = :employeeId AND ew.id.workDate"
          + " = :workDate")
  List<EmployeeWorkday> findByEmployeeUserIdAndWorkDate(
