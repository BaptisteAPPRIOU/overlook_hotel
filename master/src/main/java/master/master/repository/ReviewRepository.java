...
/**
 * Repository interface for Review entity. Provides CRUD operations and custom queries for review
 * management.
 */
@Repository
public interface ReviewRepository extends JpaRepository<Review, Long> {
  ...
  @Query(
      "SELECT r FROM Review r WHERE r.rating BETWEEN :minRating AND :maxRating "
  ...
  /**
   * Find reviews for rooms in a specific category/type. Note: This assumes Room entity has a type
   * field
   */
  @Query(
  ...
  @Query(
      "SELECT r.roomId, AVG(r.rating) as avgRating, COUNT(r) as reviewCount "
  ...
  @Query(
      "SELECT r.roomId, COUNT(r) as reviewCount, AVG(r.rating) as avgRating "
          + "FROM Review r GROUP BY r.roomId "
  ...
  @Query(
      "SELECT r FROM Review r WHERE LOWER(r.comment) LIKE LOWER(CONCAT('%', :searchText, '%')) "
          + "ORDER BY r.createdAt DESC")
  List<Review> findByCommentContaining(@Param("searchText") String searchText);

  /** Get monthly review statistics. */
  @Query(
      "SELECT YEAR(r.createdAt), MONTH(r.createdAt), COUNT(r), AVG(r.rating) "
          + "FROM Review r "
  ...
  @Query(
      "SELECT r.roomId, COUNT(DISTINCT r.authorId) as uniqueReviewers, "
  ...
  @Query(
      "SELECT r.authorId, COUNT(r) as reviewCount, AVG(r.rating) as avgRatingGiven "
          + "FROM Review r "
          + "GROUP BY r.authorId "
  ...
  @Query(
      "SELECT EXTRACT(YEAR FROM r.createdAt), EXTRACT(MONTH FROM r.createdAt), COUNT(r) as"
          + " reviewCount FROM Review r WHERE r.createdAt >= :oneYearAgo GROUP BY EXTRACT(YEAR FROM"
          + " r.createdAt), EXTRACT(MONTH FROM r.createdAt) ORDER BY EXTRACT(YEAR FROM r.createdAt)"
          + " DESC, EXTRACT(MONTH FROM r.createdAt) DESC")
