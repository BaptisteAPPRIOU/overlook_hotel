...
/**
 * Repository interface for Room entity. Provides CRUD operations and custom queries for room
 * management.
 */
@Repository
public interface RoomRepository extends JpaRepository<Room, Long> {
  ...
  List<Room> findByCapacityBetween(
      @Param("minCapacity") Integer minCapacity, @Param("maxCapacity") Integer maxCapacity);
  ...
  @Query(
      "SELECT r FROM Room r WHERE r.nextMaintenanceDate < :currentDate "
  ...
  @Query(
      "SELECT r FROM Room r WHERE r.status = master.master.domain.Room$RoomStatus.AVAILABLE ORDER"
          + " BY r.number")
