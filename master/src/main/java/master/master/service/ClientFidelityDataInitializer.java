...
/**
 * Data initialization service to fix existing clients with null fidelity points. Runs once on
 * application startup to ensure data consistency.
 */
@Component
public class ClientFidelityDataInitializer implements CommandLineRunner {

  private static final Logger logger =
      Logger.getLogger(ClientFidelityDataInitializer.class.getName());
  ...
  public void run(String... args) throws Exception {
    ...
    try {
      ...
      if (updatedCount > 0) {
        logger.info(
            String.format("Successfully initialized fidelity points for %d clients", updatedCount));
      } else {
