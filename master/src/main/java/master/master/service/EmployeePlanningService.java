 ...
 * schedules with default 35h/week planning.
 */
@Service
@Transactional
public class EmployeePlanningService {
  ...
  public EmployeePlanningDto createDefaultPlanning(Long employeeId) {
    ...
    CreatePlanningRequestDto defaultRequest =
  ...
  public EmployeePlanningDto createOrUpdatePlanning(CreatePlanningRequestDto request) {
    ...
    for (Map.Entry<Integer, Boolean> entry : weekDays.entrySet()) {
      ...
      Integer breakMinutes =
          request.getBreakDurationMinutes() != null
  ...
  public EmployeePlanningDto getEmployeePlanning(Long employeeId) {
    ...
    Map<Integer, EmployeeWorkday> workdayMap =
        workdays.stream()
            .collect(
                Collectors.toMap(
                    w -> w.getId().getWeekday(), w -> w, (existing, replacement) -> existing));
  ...
  private LocalTime getStartTimeForDay(CreatePlanningRequestDto request, Integer dayOfWeek) {
    switch (dayOfWeek) {
      case 1:
        return request.getMondayStart() != null
            ? request.getMondayStart()
            : request.getStartTime();
      case 2:
        return request.getTuesdayStart() != null
            ? request.getTuesdayStart()
            : request.getStartTime();
      case 3:
        return request.getWednesdayStart() != null
            ? request.getWednesdayStart()
            : request.getStartTime();
      case 4:
        return request.getThursdayStart() != null
            ? request.getThursdayStart()
            : request.getStartTime();
      case 5:
        return request.getFridayStart() != null
            ? request.getFridayStart()
            : request.getStartTime();
      case 6:
        return request.getSaturdayStart() != null
            ? request.getSaturdayStart()
            : request.getStartTime();
      case 7:
        return request.getSundayStart() != null
            ? request.getSundayStart()
            : request.getStartTime();
      default:
  ...
  private LocalTime getEndTimeForDay(CreatePlanningRequestDto request, Integer dayOfWeek) {
    switch (dayOfWeek) {
      case 1:
        return request.getMondayEnd() != null
            ? request.getMondayEnd()
            : request.getEndTime();
      case 2:
        return request.getTuesdayEnd() != null
            ? request.getTuesdayEnd()
            : request.getEndTime();
      case 3:
        return request.getWednesdayEnd() != null
            ? request.getWednesdayEnd()
            : request.getEndTime();
      case 4:
        return request.getThursdayEnd() != null
            ? request.getThursdayEnd()
            : request.getEndTime();
      case 5:
        return request.getFridayEnd() != null
            ? request.getFridayEnd()
            : request.getEndTime();
      case 6:
        return request.getSaturdayEnd() != null
            ? request.getSaturdayEnd()
            : request.getEndTime();
      case 7:
        return request.getSundayEnd() != null
            ? request.getSundayEnd()
            : request.getEndTime();
      default:
  ...
  /**
   * Create or update hourly planning for an employee. This method handles the detailed hour-by-hour
   * scheduling.
   */
  ...
  /**
   * Save hourly planning for an employee for a specific week. This method creates or updates
   * workday entries based on hourly schedule.
   */
  public boolean saveHourlyPlanning(master.master.web.rest.dto.WeeklyHourlyPlanningDto request) {
    try {
      ...
      if (request.getSchedule() != null && !request.getSchedule().isEmpty()) {
        for (Map.Entry<String, List<Integer>> dayEntry : request.getSchedule().entrySet()) {
          ...
          if (hours != null && !hours.isEmpty()) {
            ...
            int endHour =
                hours.stream().max(Integer::compareTo).orElse(17)
                    + 1; // +1 because end hour is exclusive
      ...
      throw new ResponseStatusException(
          INTERNAL_SERVER_ERROR, "Failed to save hourly planning: " + e.getMessage());
    }
  }
    ...
    try {
      ...
      for (int dayOffset = 0; dayOffset < 7; dayOffset++) {
        ...
        workdayRepository
            ...
            .ifPresent(
                workday -> {
                  if (workday.getPlannedStartTime() != null
      ...
      throw new ResponseStatusException(
          INTERNAL_SERVER_ERROR, "Failed to get hourly planning: " + e.getMessage());
    }
  }
  ...
  public Map<Long, Map<String, List<Map<String, Object>>>> getWeeklyScheduleForPlanning(
    ...
    for (EmployeeWorkday workday : workdays) {
      ...
      System.out.println(
          "Processing workday: Employee "
              + employeeId
              + " on "
              + dateStr
              + " from "
              + workday.getPlannedStartTime()
              + " to "
              + workday.getPlannedEndTime());
      ...
      schedule
          ...
          .add(
              Map.of(
                  ...
                  shiftType,
                  "weekday",
                  workday.getId().getWeekday(),
                  "startTime",
                  workday.getPlannedStartTime() != null
                      ? workday.getPlannedStartTime().toString()
                      : "",
                  "endTime",
                  workday.getPlannedEndTime() != null ? workday.getPlannedEndTime().toString() : "",
                  "status",
                  "SCHEDULED"));
    }
  ...
  public Map<String, Object> createShiftFromMap(Map<String, Object> shiftData) {
    try {
      ...
      List<EmployeeWorkday> existingShifts =
          workdayRepository.findByEmployeeUserIdAndWorkDate(employeeId, date);

      System.out.println(
          "Found "
              + existingShifts.size()
              + " existing shifts for employee "
              + employeeId
              + " on "
              + date);
      ...
      for (EmployeeWorkday existingShift : existingShifts) {
        ...
        // (New start during existing shift OR new end during existing shift OR new shift
        // encompasses existing shift)
        boolean overlaps =
        ...
        if (overlaps) {
          System.out.println(
              "Potential time conflict detected with existing shift: "
                  + existingStart
                  + " - "
                  + existingEnd);
          System.out.println("New shift time: " + startTime + " - " + endTime);
          // Uncomment to enforce no overlapping shifts:
          // throw new RuntimeException("This shift overlaps with an existing shift for this
      ...
      System.out.println(
          "Creating shift with unique weekday ID: "
              + uniqueWeekday
              + " (base weekday: "
              + weekday
              + ", offset: "
  ...
  /**
   * Delete a specific shift by employee ID, date, and weekday. The weekday parameter helps identify
   * which shift to delete when there are multiple shifts on the same day.
   */
  @Transactional
  public void deleteSpecificShift(Long employeeId, LocalDate date, Integer weekday) {
    System.out.println(
        "Deleting shift for employee "
            + employeeId
            + " on "
            + date
            + " with weekday code "
    ...
    // If direct deletion failed or weekday was null, try to find all shifts for this employee and
    // day
    List<EmployeeWorkday> workdays =
        workdayRepository.findByEmployeeUserIdAndWorkDate(employeeId, date);
    System.out.println(
        "Found " + workdays.size() + " shifts for employee " + employeeId + " on " + date);
  ...
  /**
   * Determine shift type based on weekday value. We use this method to interpret the position from
   * the weekday value since we don't have a position field in EmployeeWorkday.
   */
