...
/**
 * Service for managing employee workday configurations. Handles CRUD operations for employee work
 * schedules and provides schedule views.
 *
 * <p>This service focuses on the workday configuration aspect, while ScheduleService handles the
 * actual schedule generation and timesheet operations.
 */
@Service
@Transactional
public class EmployeeWorkdayService {
  ...
  /**
   * Get configured workdays for an employee. Returns list of weekday numbers (1=Monday, 7=Sunday).
   */
  public List<Integer> getWorkdaysByEmployeeId(Long employeeId) {
    ...
    try {
      return workdayRepository.findByEmployeeUserId(employeeId).stream()
      ...
      System.err.println(
          "Error fetching workdays for employee " + employeeId + ": " + e.getMessage());
      return new ArrayList<>();
    }
  }

  /** Set workdays for an employee. Replaces existing workday configuration with new one. */
  ...
  /**
   * @deprecated Use ScheduleService.getWeeklySchedules() instead. This method is kept for backward
   *     compatibility.
   */
  @Deprecated
  public List<WeeklyScheduleDto> getWeeklySchedules() {
    ...
    for (Employee employee : employees) {
      ...
      try {
        ...
        System.err.println(
            "Error processing workdays for employee "
                + employee.getUserId()
                + ": "
                + e.getMessage());
      }
  ...
  /**
   * @deprecated Use ScheduleService.getMonthlySchedules() instead. This method is kept for backward
   *     compatibility.
   */
  @Deprecated
  public List<MonthlyScheduleDto> getMonthlySchedules(Month targetMonth) {
    ...
    for (Employee employee : employees) {
      ...
      try {
        ...
        System.err.println(
            "Error processing monthly schedule for employee "
                + employee.getUserId()
                + ": "
                + e.getMessage());
      }
  ...
  /**
   * @deprecated Use ScheduleService.getSchedulesByDateRange() instead. This method is kept for
  ...
  public List<DateRangeScheduleDto> getSchedulesInRange(LocalDate start, LocalDate end) {
    ...
    for (EmployeeWorkday workday : allWorkdays) {
      ...
      DateRangeScheduleDto dto =
          scheduleMap.computeIfAbsent(
              employeeId,
              k -> {
  ...
  public boolean hasWorkdaysConfigured(Long employeeId) {
    ...
    try {
      ...
      System.err.println(
          "Error checking workdays for employee " + employeeId + ": " + e.getMessage());
      return false;
    }
  }
  ...
  public void bulkUpdateWorkdays(Map<Long, List<Integer>> employeeWorkdays) {
    ...
    for (Map.Entry<Long, List<Integer>> entry : employeeWorkdays.entrySet()) {
      try {
        ...
        System.err.println(
            "Error updating workdays for employee " + entry.getKey() + ": " + e.getMessage());
