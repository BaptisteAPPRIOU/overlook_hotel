...
/**
 * Service for managing employee leave requests. Handles creation, approval, rejection, and
 * retrieval of leave requests.
 */
@Service
@Transactional
public class LeaveRequestService {
  ...
  public LeaveRequestDto createLeaveRequest(CreateLeaveRequestDto request) {
    ...
    if (!overlapping.isEmpty()) {
      throw new IllegalArgumentException(
          "Leave request overlaps with existing approved/pending leave");
    }
  ...
  public List<LeaveRequestDto> getPendingLeaveRequests() {
    List<LeaveRequest> pendingRequests =
        leaveRequestRepository.findByStatusOrderByCreatedAtDesc(LeaveRequest.LeaveStatus.PENDING);
  ...
  /** Get count of pending leave requests. Used for dashboard statistics. */
  @Transactional(readOnly = true)
  ...
  public List<LeaveRequestDto> getEmployeeLeaveRequests(Long employeeId) {
    List<LeaveRequest> employeeRequests =
        leaveRequestRepository.findByEmployeeIdOrderByCreatedAtDesc(employeeId);
    return employeeRequests.stream().map(this::mapToDto).collect(Collectors.toList());
  }
  ...
  public Double getLeaveBalance(Long employeeId, String leaveType) {
    ...
    // Long usedDays = leaveRequestRepository.getTotalLeaveDaysUsed(employeeId,
    // leaveType.toUpperCase(), currentYear);
  ...
  public boolean hasOverlappingLeave(Long employeeId, LocalDate startDate, LocalDate endDate) {
    List<LeaveRequest> overlapping =
        leaveRequestRepository.findOverlappingLeaveRequests(employeeId, startDate, endDate);
