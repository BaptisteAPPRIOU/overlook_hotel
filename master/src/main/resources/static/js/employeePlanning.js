  ...
  return {
    ...
    getEmployeeName(employee) {
      if (employee.employeeName) {
        ...
        return (
          "Employee #" + (employee.employeeId || employee.userId || "Unknown")
        );
      }
    },
    ...
    getEmployeeHours(employee) {
      ...
      if (this.schedule[employeeId]) {
        ...
        for (let i = 0; i < allShifts.length; i++) {
          ...
          const nextDay =
            nextShift && this.isConsecutiveDay(shift.date, nextShift.date);
          ...
          if (shift.startTime && shift.endTime) {
            try {
              ...
              if (
                ...
                nextDay &&
                shift.endTime === "24:00" &&
                nextShift.startTime === "00:00"
              ) {
                // Don't double count the midnight minute when shifts are consecutive across days
                totalHours += hours + mins / 60 - 1 / 60;
                console.log(
                  `Consecutive overnight shifts detected: ${shift.date} ${shift.startTime}-${shift.endTime} and ${nextShift.date} ${nextShift.startTime}-${nextShift.endTime}`,
                );
              } else {
                // Normal calculation
    ...
    async loadSchedule() {
      try {
        ...
        const response = await fetch(
          "/api/planning/week?start=" + startDateStr,
          {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("jwtToken"),
            },
          ...
          Object.entries(rawSchedule).forEach(([employeeId, employeeDates]) => {
            ...
            Object.entries(employeeDates).forEach(([dateStr, shifts]) => {
              ...
              const normalizedShifts = shifts.map((shift) => {
                ...
                console.log(
                  `Processing shift for employee ${employeeId} on ${dateStr}: ID=${normalizedShift.id}, Type=${normalizedShift.type}`,
                );
              ...
              console.log(
                `Loaded ${normalizedShifts.length} shifts for employee ${employeeId} on ${dateStr}:`,
                normalizedShifts,
              );
    ...
    async addShift(employeeId, date, shiftType, startTime, endTime) {
      ...
      const employee = this.employees.find(
        (e) => e.userId == employeeId || e.employeeId == employeeId,
      );
      if (employee) {
        // If the employee has a user role of ADMIN or MANAGER, set them as MANAGER type
        if (
          employee.user &&
          (employee.user.role === "ADMIN" || employee.user.role === "MANAGER")
        ) {
          normalizedShiftType = "MANAGER";
        } else {
    ...
    openEditShiftModal(shift, employee, date) {
      ...
      const saveShiftChangesBtn = document.getElementById(
        "saveShiftChangesBtn",
      );

      // Always use the shiftId we've constructed to ensure it has the weekday
      deleteShiftBtn.onclick = () =>
        this.deleteShift(shiftId, employee.userId || employee.employeeId, date);
        ...
        try {
          ...
          if (shiftId && typeof shiftId === "string") {
            ...
            if (shiftId.weekday) {
              ...
              console.log(
                "Found weekday directly in shift object:",
                weekdayValue,
              );
            } else if (
              shiftId.id &&
              typeof shiftId.id === "string" &&
              shiftId.id.includes(",")
            ) {
              ...
              if (idParts.length > 1) {
                weekdayValue = idParts[1].trim();
                console.log(
                  "Found weekday in shift.id property:",
                  weekdayValue,
                );
              }
            }
    ...
    async updateShift() {
      ...
      if (!employeeId || employeeId === "") {
        console.log(
          "Employee ID not found in form data, trying alternative sources",
        );
        ...
        if (employeeIdElem && employeeIdElem.value) {
          ...
          const employeeIdHidden = document.querySelector(
            '#editShiftModal input[name="employeeId"]',
          );
      ...
      try {
        ...
        if (shiftId) {
          ...
          try {
            ...
            console.warn(
              "Could not delete existing shift, continuing with create:",
              deleteError,
            );
          }
        }
        ...
        try {
          ...
          if (response.ok) {
            ...
            alert(
              `Error updating shift (${response.status}). Please try again.`,
            );
          }
        } catch (networkError) {
          console.error("Network error when updating shift:", networkError);
          alert(
            "Network error when updating shift. Please check your connection and try again.",
          );
        }
      } catch (error) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.validateShiftTimes = function () {
    ...
    const submitButton = document.querySelector(
      "#addShiftForm + .modal-footer .btn-primary",
    );
  ...
  if (addShiftForm) {
    addShiftForm.addEventListener("submit", async function (e) {
      ...
      if (planningInterface && planningInterface.__x) {
        ...
        await planningInterface.__x.data.addShift(
          employeeId,
          dateObj,
          position,
          startTime,
