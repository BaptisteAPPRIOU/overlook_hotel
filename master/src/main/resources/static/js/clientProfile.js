...
async function calculateFidelityStats() {
  try {
    ...
    const reservationsResponse = await fetch(
      "/api/v1/clients/me/reservations",
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    if (reservationsResponse.ok) {
      ...
      const totalNights = reservations
        .filter(
          (res) => res.payed && new Date(res.reservationDateEnd) < new Date(),
        )
        .reduce((total, res) => {
      ...
      const currentPoints =
        parseInt(document.getElementById("currentPoints").textContent) || 0;
...
function calculateTotalEarnedPoints(reservations) {
  return reservations
    ...
    .reduce((total, res) => {
      ...
      const reservationMade = new Date(
        res.createdAt || res.reservationDateStart,
      );
      const daysBefore = Math.ceil(
        (start - reservationMade) / (1000 * 60 * 60 * 24),
      );
      if (daysBefore >= 30) {
        points += 25;
      }
  ...
  try {
    ...
    if (result.success) {
      ...
      document
        ...
        .classList.add("points-animation");
